...
def plot_gen(generation):
    ...
    #     avg_defl = np.average(design[2:7])
    #     plot_x.append(avg_defl)
    #     plot_y.append(design[8])
    # plt.plot(plot_x, plot_y, 'o')
    ax.plot(generation[:, 3], generation[:, 8], 'o')
    ax.set_xlabel('d2x')
    ax2.plot(generation[:, 6], generation[:, 8], 'o')
    ax2.set_xlabel('d4x')
    ax.relim()
    ax2.relim()
    plt.pause(0.01)
...
if __name__ == "__main__":
    ...
    gen_opt = MultiObjectiveOptimizer(opt_vals, solve_truss, n_generations=500, population_size=100, n_objectives=8,
                                      generation_func=plot_gen, constraint=truss_const, constraint_func_input='full')
    opts = gen_opt.find_min()

    plt.ioff()
    for opt_des in opts[0:50, 9:]:
        plot_truss(opt_des)
